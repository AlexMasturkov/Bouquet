...
// Get all Event Types From API 
async function getQuotes() {
    loading();
    const apiUrl = '/Admin/EventType/GetAll';
    try {
...
//First call to get all EventTypes
getQuotes();

//Create Html Grid Elements of EventTypes
function buildEvent() {
    ...
    if (window.innerWidth <= 480) {
        for (let element of eventColumns) {
         ...
         <div class="grid-three-column-left font-14 ${iterator} p-5 " style="height:58px; ">
                        ...
                        <div class="item-row "> <i class="icon-trash-square i-btn-sm i-border-danger3 " onclick=Delete("/Admin/EventType/Delete/${element.id}")></i></div>
                        <a class="item-row " href="/Admin/EventType/Upsert/${element.id}"> <i class="icon-edit-in-square i-btn-sm i-border-success3"></i></a>                        
        ...
        for (let element of eventColumns) {
         ...
         <div class="grid-three-column-left font-16 ${iterator} p-5 " style="height:58px; ">
                        ...
                        <div class="item-row "> <i class="icon-trash-square i-btn-sm i-border-danger3 " style="transform:scale(1.2); " onclick=Delete("/Admin/EventType/Delete/${element.id}")></i></div>
                        <a class="item-row " href="/Admin/EventType/Upsert/${element.id}"> <i class="icon-edit-in-square i-btn-sm i-border-success3" style="transform:scale(1.2); "></i></a>                        
                    </div> <hr class="width-max" />`;
            eventsText = eventsText + eventText;
        }
        columns.innerHTML = '<hr class="width-max" />' + eventsText + '</div>';
